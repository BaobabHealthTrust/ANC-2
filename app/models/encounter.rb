class Encounter < ActiveRecord::Base
  ...
  def after_save
    ...
    result = RestClient.post("#{protocal}://#{settings["dde_username"]}:#{settings["dde_password"]}@#{settings["dde_server"]}/ajax_log",
                             {"person"=>{"national_id"=>"#{identifier}","application"=>"#{settings["application_name"]}","site_code"=>"#{settings["site_code"]}"}},
                             {:accept => :json}) if !identifier.blank?
  end
